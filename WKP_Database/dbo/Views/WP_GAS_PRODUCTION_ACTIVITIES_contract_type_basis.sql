...
FROM         (SELECT     Contract_Type, SUM(CAST(Current_Actual_Year AS int)) AS Total_GAS_Production_by_company, Year_of_WP
                       FROM          dbo.GAS_PRODUCTION_ACTIVITIES
                       GROUP BY Contract_Type, Year_of_WP) AS a INNER JOIN
                          (SELECT     SUM(CAST(Current_Actual_Year AS int)) AS Total_GAS_Production_by_year, Year_of_WP
